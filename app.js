...
// Per modificare condomini, posti o cicli: edita solo data.json, non questo file.
...
async function loadData() {
  try {
    ...
    groupA            = d.groupA;
    groupB            = d.groupB;
    smallGroups       = d.smallGroups;
    cycle             = d.cycle;
    periodTemplates   = d.periodTemplates;
    realSpotPositions = d.realSpotPositions;
  } catch (err) {
    console.error("Impossibile caricare data.json:", err);
    document.body.innerHTML =
      '<p style="padding:2rem;font-family:sans-serif;color:red">' +
      "Errore nel caricamento dei dati. Ricarica la pagina o controlla la connessione.</p>";
    throw err;
  }
}
...
// spotRights viene inizializzato in DOMContentLoaded dopo loadData()
let spotRights = new Map();
...
document.addEventListener("DOMContentLoaded", async ()=>{
  ...
  // buildSpotRights dipende dai dati: va chiamato dopo il caricamento
  spotRights = buildSpotRights();
